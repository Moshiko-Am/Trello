...
export default {
	...
	methods: {
		...
		updateCover() {},
		addUser(userId) {
			const member = this.board.members.find(
		...
		addCl(checklist) {
			...
			const clsCopy = JSON.parse(
				JSON.stringify(this.cardToEdit.checklists)
			);
		...
		updateAttachments(attachments) {
			...
			if (
				...
				this.cardToEdit.attachments.every(
				...
				return this.cardToEdit.cover.type === 'attachment'
					? `background-color: rgb(${
							this.cardToEdit.attachments[
								this.cardToEdit.cover.attachmentIdx
							].props.colorArray[0]
					  },${
							this.cardToEdit.attachments[
								this.cardToEdit.cover.attachmentIdx
